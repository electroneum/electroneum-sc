...
import (
	...
	"github.com/electroneum/electroneum-sc/accounts/usbwallet"
	...
	"github.com/electroneum/electroneum-sc/signer/storage"
)
...
func (api *SignerAPI) SignTransaction(ctx context.Context, args apitypes.SendTxArgs, methodSelector *string) (*ethapi.SignTransactionResult, error) {
	...
	var unsignedTx = result.Transaction.ToTransaction() // the parent function is only for Clef. No need to support ETN transactions right now. You can't grab pkey for signature with no backend without extra faff
