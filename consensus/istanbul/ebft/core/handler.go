...
func (c *core) Stop() error {
	c.logger.Info("EBFT: stopping...")
	c.stopTimer()
	c.unsubscribeEvents()

	// Make sure the handler goroutine exits
	c.handlerWg.Wait()
	c.logger.Info("EBFT: stopped")
	return nil
}
...
// handleEvents starts main ebft handler loop that processes all incoming messages
// sequentially. Each time a message is processed, internal EBFT state is mutated
	...
	for {
		...
		case event, ok := <-c.events.Chan():
			...
			case backlogEvent:
				...
				if err != nil {
					c.logger.Error("EBFT: can not encode backlog message", "err", err)
					continue
				}
...
func (c *core) handleEncodedMsg(code uint64, data []byte) error {
	...
	if _, ok := ebfttypes.MessageCodes()[code]; !ok {
		logger.Error("EBFT: invalid message event code")
		return fmt.Errorf("invalid message event code %v", code)
	}

	// Decode data into a EBFTMessage
	m, err := ebfttypes.Decode(code, data)
	if err != nil {
		logger.Error("EBFT: invalid message", "err", err)
		return err
	}
...
func (c *core) handleDecodedMessage(m ebfttypes.EBFTMessage) error {
	view := m.View()
...
func (c *core) deliverMessage(m ebfttypes.EBFTMessage) error {
	var err error

	switch m.Code() {
	case ebfttypes.PreprepareCode:
		err = c.handlePreprepareMsg(m.(*ebfttypes.Preprepare))
	case ebfttypes.PrepareCode:
		err = c.handlePrepare(m.(*ebfttypes.Prepare))
	case ebfttypes.CommitCode:
		err = c.handleCommitMsg(m.(*ebfttypes.Commit))
	case ebfttypes.RoundChangeCode:
		err = c.handleRoundChange(m.(*ebfttypes.RoundChange))
	default:
		c.logger.Error("EBFT: invalid message code", "code", m.Code())
...
func (c *core) handleTimeoutMsg() {
	...
	logger.Warn("[Consensus|EBFT]: Round reached timeout", "pr", c.current.preparedRound)
	c.startNewRound(nextRound)
	logger.Trace("EBFT: TIMER CHANGED ROUND", "pr", c.current.preparedRound)
...
func (c *core) verifySignatures(m ebfttypes.EBFTMessage) error {
	logger := c.currentLogger(true, m)

	// Anonymous function to verify the signature of a single message or payload
	verify := func(m ebfttypes.EBFTMessage) error {
		payload, err := m.EncodePayloadForSigning()
		if err != nil {
			logger.Error("EBFT: invalid message payload", "err", err)
			return err
		}
		source, err := c.validateFn(payload, m.Signature())
		if err != nil {
			logger.Error("EBFT: invalid message signature", "err", err)
...
func (c *core) currentLogger(state bool, msg ebfttypes.EBFTMessage) log.Logger {
	logCtx := []interface{}{}
	if c.current != nil {
...
func withMsg(logger log.Logger, msg ebfttypes.EBFTMessage) log.Logger {
	return logger.New(
		"msg.code", msg.Code(),
		"msg.source", msg.Source().String(),
		"msg.round", msg.View().Round.Uint64(),
