...
var (
	roundMeter     = metrics.NewRegisteredMeter("consensus/istanbul/ebft/core/round", nil)
	sequenceMeter  = metrics.NewRegisteredMeter("consensus/istanbul/ebft/core/sequence", nil)
	consensusTimer = metrics.NewRegisteredTimer("consensus/istanbul/ebft/core/consensus", nil)
)
...
type core struct {
	...
	EBFTPreparedPrepares []*ebfttypes.Prepare
...
func (c *core) startNewRound(round *big.Int) {
	...
	if c.current == nil {
		...
		logger.Debug("EBFT: catch up last block proposal")
	} else if lastProposal.Number().Cmp(big.NewInt(c.current.Sequence().Int64()-1)) == 0 {
		if round.Cmp(common.Big0) == 0 {
			// same seq and round, don't need to start new round
			logger.Debug("EBFT: same round, no need to start new round")
			return
		} else if round.Cmp(c.current.Round()) < 0 {
			logger.Warn("EBFT: next round is inferior to current round")
			return
		}
		roundChange = true
	} else {
		logger.Warn("EBFT: next sequence is before last block proposal")
		return
	}
	...
	oldLogger.Trace("EBFT: start new round", "next.round", newView.Round, "next.seq", newView.Sequence, "next.proposer", c.valSet.GetProposer(), "next.valSet", c.valSet.List(), "next.size", c.valSet.Size(), "next.IsProposer", c.IsProposer())
}
...
func (c *core) newRoundChangeTimer() {
	...
	if maxRequestTimeout > time.Duration(0) {
		...
		if timeout < baseTimeout {
			c.currentLogger(true, nil).Error("EBFT: Possible request timeout overflow detected, setting timeout value to maxRequestTimeout",
