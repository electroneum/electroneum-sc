...
func testParameterizedCase(
	...
	roundChangeMessages := make([]*ebfttypes.SignedRoundChangePayload, 0)
	for index, validator := range validatorSet.List() {
		var m *ebfttypes.SignedRoundChangePayload
		if index < rcForNil {
	...
	// PREPARE messages
	prepareMessages := make([]*ebfttypes.Prepare, 0)
	for index, validator := range validatorSet.List() {
		var m *ebfttypes.Prepare
...
func createRoundChangeMessage(from common.Address, round int64, preparedRound int64, preparedBlock istanbul.Proposal) *ebfttypes.SignedRoundChangePayload {
	m := ebfttypes.NewRoundChange(big.NewInt(1), big.NewInt(1), big.NewInt(preparedRound), preparedBlock, false)
	m.SetSource(from)
	return &m.SignedRoundChangePayload
}

func createPrepareMessage(from common.Address, round int64, preparedBlock istanbul.Proposal) *ebfttypes.Prepare {
	return ebfttypes.NewPrepareWithSigAndSource(big.NewInt(1), big.NewInt(round), preparedBlock.Hash(), nil, from)
}
