...
func newRoundState(view *istanbul.View, validatorSet istanbul.ValidatorSet, preprepare *ebfttypes.Preprepare, preparedRound *big.Int, preparedBlock istanbul.Proposal, pendingRequest *Request, hasBadProposal func(hash common.Hash) bool) *roundState {
	return &roundState{
		round:      view.Round,
		...
		EBFTPrepares:   newEBFTMsgSet(validatorSet),
		EBFTCommits:    newEBFTMsgSet(validatorSet),
...
type roundState struct {
	round      *big.Int
	sequence   *big.Int
	Preprepare *ebfttypes.Preprepare

	EBFTPrepares *ebftMsgSet
	EBFTCommits  *ebftMsgSet
...
func (s *roundState) SetPreprepare(preprepare *ebfttypes.Preprepare) {
	s.mu.Lock()
	defer s.mu.Unlock()
