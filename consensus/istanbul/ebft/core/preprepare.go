...
func (c *core) sendPreprepareMsg(request *Request) {
	...
	if c.current.Sequence().Cmp(request.Proposal.Number()) == 0 && c.IsProposer() {
		...
		preprepare := ebfttypes.NewPreprepare(curView.Sequence, curView.Round, request.Proposal)
		preprepare.SetSource(c.Address())

		c.logger.Info("[Consensus|EBFT]: Proposing new block", "sequence", curView.Sequence.Uint64())

		// Sign payload
		encodedPayload, err := preprepare.EncodePayloadForSigning()
		if err != nil {
			withMsg(logger, preprepare).Error("EBFT: failed to encode payload of PRE-PREPARE message", "err", err)
			return
		}
		signature, err := c.backend.Sign(encodedPayload)
		if err != nil {
			withMsg(logger, preprepare).Error("EBFT: failed to sign PRE-PREPARE message", "err", err)
			return
		}
		...
		if request.RCMessages != nil {
			preprepare.JustificationRoundChanges = make([]*ebfttypes.SignedRoundChangePayload, 0)
			for _, m := range request.RCMessages.Values() {
				preprepare.JustificationRoundChanges = append(preprepare.JustificationRoundChanges, &m.(*ebfttypes.RoundChange).SignedRoundChangePayload)
				withMsg(logger, preprepare).Trace("EBFT: add ROUND-CHANGE justification", "rc", m.(*ebfttypes.RoundChange).SignedRoundChangePayload)
			}
			withMsg(logger, preprepare).Trace("EBFT: extended PRE-PREPARE message with ROUND-CHANGE justifications", "justifications", preprepare.JustificationRoundChanges)
		}
		...
		if request.PrepareMessages != nil {
			...
			withMsg(logger, preprepare).Trace("EBFT: extended PRE-PREPARE message with PREPARE justification", "justification", preprepare.JustificationPrepares)
		}

		// RLP-encode message
		payload, err := rlp.EncodeToBytes(&preprepare)
		if err != nil {
			withMsg(logger, preprepare).Error("EBFT: failed to encode PRE-PREPARE message", "err", err)
			return
		}
		...
		logger.Trace("EBFT: broadcast PRE-PREPARE message", "payload", hexutil.Encode(payload))

		// Broadcast RLP-encoded message
		if err = c.backend.Broadcast(c.valSet, preprepare.Code(), payload); err != nil {
			logger.Error("EBFT: failed to broadcast PRE-PREPARE message", "err", err)
			return
		}
...
func (c *core) handlePreprepareMsg(preprepare *ebfttypes.Preprepare) error {
	...
	c.logger.Trace("EBFT: handle PRE-PREPARE message")

	// Validates PRE-PREPARE message comes from current proposer
	if !c.valSet.IsProposer(preprepare.Source()) {
		logger.Warn("[Consensus|EBFT]: Ignore PRE-PREPARE message from non proposer", "proposer", c.valSet.GetProposer().Address())
	...
	if preprepare.Round.Uint64() > 0 {
		if err := isJustified(preprepare.Proposal, preprepare.JustificationRoundChanges, preprepare.JustificationPrepares, c.QuorumSize()); err != nil {
			logger.Warn("[Consensus|EBFT]: Invalid PRE-PREPARE message justification", "err", err)
	...
	if duration, err := c.backend.Verify(preprepare.Proposal); err != nil {
		...
		if err == consensus.ErrFutureBlock {
			logger.Warn("[Consensus|EBFT]: PRE-PREPARE block proposal is in the future (will be treated again later)", "duration", duration)
			...
			logger.Warn("[Consensus|EBFT]: Invalid PRE-PREPARE block proposal", "err", err)
		}
	...
	if c.state == StateAcceptRequest {
		c.logger.Trace("EBFT: accepted PRE-PREPARE message")
		...
		c.cleanLogger.Info("[Consensus|EBFT]: <- Received PRE-PREPARE message from proposer", "author", preprepare.Source())
