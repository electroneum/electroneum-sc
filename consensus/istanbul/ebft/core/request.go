...
// (this is the starting of the EBFT validation process)
...
func (c *core) handleRequest(request *Request) error {
	...
	if err := c.checkRequestMsg(request); err != nil {
		if err == errInvalidMessage {
			logger.Error("EBFT: invalid request")
			return err
		}
		logger.Error("EBFT: unexpected request", "err", err, "number", request.Proposal.Number(), "hash", request.Proposal.Hash())
		return err
	}
...
func (c *core) storeRequestMsg(request *Request) {
	...
	logger.Trace("EBFT: store block proposal request for future treatment")
...
// processPendingRequests is called each time EBFT state is re-initialized
...
func (c *core) processPendingRequests() {
	...
	logger := c.currentLogger(true, nil)
	logger.Debug("EBFT: lookup for pending block proposal requests")

	for !(c.pendingRequests.Empty()) {
		m, prio := c.pendingRequests.Pop()
		r, ok := m.(*Request)
		if !ok {
			logger.Error("EBFT: malformed pending block proposal request, skip", "msg", m)
			continue
		}
		...
		if err != nil {
			if err == errFutureMessage {
				logger.Trace("EBFT: stop looking up for pending block proposal request")
				c.pendingRequests.Push(m, prio)
				break
			}
			logger.Trace("EBFT: skip pending invalid block proposal request", "number", r.Proposal.Number(), "hash", r.Proposal.Hash(), "err", err)
			continue
		}
		logger.Debug("EBFT: found pending block proposal request", "proposal.number", r.Proposal.Number(), "proposal.hash", r.Proposal.Hash())
