...
var msgPriority = map[uint64]int{
	ebfttypes.PreprepareCode: 1,
	ebfttypes.CommitCode:     2,
	ebfttypes.PrepareCode:    3,
}

// checkMessage checks that a message matches our current EBFT state
//
...
func (c *core) checkMessage(msgCode uint64, view *istanbul.View) error {
	...
	if msgCode == ebfttypes.RoundChangeCode {
	...
	case StateAcceptRequest:
		...
		if msgCode > ebfttypes.PreprepareCode {
			return errFutureMessage
		}
		return nil
	case StatePreprepared:
		...
		if msgCode < ebfttypes.PrepareCode {
			return errInvalidMessage
		} else if msgCode > ebfttypes.PrepareCode {
			return errFutureMessage
		}
		return nil
	case StatePrepared:
		...
		if msgCode < ebfttypes.CommitCode {
			return errInvalidMessage
		}
		return nil
	case StateCommitted:
...
func (c *core) addToBacklog(msg ebfttypes.EBFTMessage) {
	logger := c.currentLogger(true, msg)

	src := msg.Source()
	if src == c.Address() {
		logger.Warn("EBFT: backlog from self")
		return
	}

	logger.Trace("EBFT: new backlog message", "backlogs_size", len(c.backlogs))
...
func (c *core) processBacklog() {
	...
	for srcAddress, backlog := range c.backlogs {
		...
		logger.Trace("EBFT: process backlog")
		...
		for !(backlog.Empty() || isFuture) {
			...
			if err != nil {
				if err == errFutureMessage {
					...
					logger.Trace("EBFT: stop processing backlog", "msg", m)
					backlog.Push(m, prio)
					isFuture = true
					break
				}
				logger.Trace("EBFT: skip backlog message", "msg", m, "err", err)
				continue
			}
			logger.Trace("EBFT: post backlog event", "msg", m)
