...
func (c *core) broadcastRoundChange(round *big.Int) {
	...
	roundChange := ebfttypes.NewRoundChange(c.current.Sequence(), round, c.current.preparedRound, c.current.preparedBlock, hasBadProposal)
...
func (c *core) handleRoundChange(roundChange *ebfttypes.RoundChange) error {
	...
	logger.Trace("EBFT: handle ROUND-CHANGE message", "higherRoundChanges.count", num, "currentRoundChanges.count", currentRoundMessages)

	// Add ROUND-CHANGE message to message set
	if view.Round.Cmp(currentRound) >= 0 {
		var prepareMessages []*ebfttypes.Prepare = nil
		var pr *big.Int = nil
	...
	if num == c.valSet.F()+1 {
		...
		rcSignedPayloads := make([]*ebfttypes.SignedRoundChangePayload, 0)
		for _, m := range roundChangeMessages.Values() {
			rcMsg := m.(*ebfttypes.RoundChange)
		...
		if err := isJustified(proposal, rcSignedPayloads, prepareMessages, c.QuorumSize()); err != nil {
			logger.Error("EBFT: invalid ROUND-CHANGE message justification", "err", err)
			return nil
		}
		...
		logger.Trace("EBFT: accepted ROUND-CHANGE messages")
	}
...
func newRoundChangeSet(valSet istanbul.ValidatorSet) *roundChangeSet {
	return &roundChangeSet{
		validatorSet:         valSet,
		roundChanges:         make(map[uint64]*ebftMsgSet),
		prepareMessages:      make(map[uint64][]*ebfttypes.Prepare),
...
type roundChangeSet struct {
	...
	roundChanges         map[uint64]*ebftMsgSet
	prepareMessages      map[uint64][]*ebfttypes.Prepare
...
func (rcs *roundChangeSet) NewRound(r *big.Int) {
	...
	if rcs.roundChanges[round] == nil {
		rcs.roundChanges[round] = newEBFTMsgSet(rcs.validatorSet)
	}
	if rcs.prepareMessages[round] == nil {
		rcs.prepareMessages[round] = make([]*ebfttypes.Prepare, 0)
	}
}

// Add adds the round and message into round change set
func (rcs *roundChangeSet) Add(r *big.Int, msg ebfttypes.EBFTMessage, preparedRound *big.Int, preparedBlock istanbul.Proposal, prepareMessages []*ebfttypes.Prepare, quorumSize int) error {
	rcs.mu.Lock()
	defer rcs.mu.Unlock()
	...
	if preparedRound != nil && (rcs.highestPreparedRound[round] == nil || preparedRound.Cmp(rcs.highestPreparedRound[round]) > 0) {
		roundChange := msg.(*ebfttypes.RoundChange)
