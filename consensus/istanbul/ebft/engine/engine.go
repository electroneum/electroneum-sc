...
func writeCommittedSeals(committedSeals [][]byte) ApplyEBFTExtra {
	return func(ebftExtra *types.EBFTExtra) error {
		...
		ebftExtra.CommittedSeal = make([][]byte, len(committedSeals))
		copy(ebftExtra.CommittedSeal, committedSeals)
...
func writeRoundNumber(round *big.Int) ApplyEBFTExtra {
	return func(ebftExtra *types.EBFTExtra) error {
		ebftExtra.Round = uint32(round.Uint64())
		return nil
	}
}
...
func (e *Engine) verifyHeader(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header, validators istanbul.ValidatorSet) error {
	...
	if _, err := types.ExtractEBFTExtra(header); err != nil {
...
func (e *Engine) verifyCommittedSeals(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header, validators istanbul.ValidatorSet) error {
	...
	extra, err := types.ExtractEBFTExtra(header)
	if err != nil {
		return err
	}
...
func WriteValidators(validators []common.Address) ApplyEBFTExtra {
	return func(ebftExtra *types.EBFTExtra) error {
		ebftExtra.Validators = validators
		return nil
	}
}
...
func sigHash(header *types.Header) (hash common.Hash) {
	...
	rlp.Encode(hasher, types.EBFTFilteredHeader(header))
	hasher.Sum(hash[:0])
	return hash
}
...
func PrepareCommittedSeal(header *types.Header, round uint32) []byte {
	...
	return h.EBFTHashWithRoundNumber(round).Bytes()
}
...
func WriteVote(candidate common.Address, authorize bool) ApplyEBFTExtra {
	return func(ebftExtra *types.EBFTExtra) error {
...
func (e *Engine) ReadVote(header *types.Header) (candidate common.Address, authorize bool, err error) {
	ebftExtra, err := getExtra(header)
	if err != nil {
		return common.Address{}, false, err
	}

	var vote *types.ValidatorVote
	if ebftExtra.Vote == nil {
...
func getExtra(header *types.Header) (*types.EBFTExtra, error) {
	if len(header.Extra) < types.IstanbulExtraVanity {
		// In this scenario, the header extradata only contains client specific information, hence create a new ebftExtra and set vanity
		vanity := append(header.Extra, bytes.Repeat([]byte{0x00}, types.IstanbulExtraVanity-len(header.Extra))...)
		return &types.EBFTExtra{
			VanityData:    vanity,
	...
	return types.ExtractEBFTExtra(header)
}

func setExtra(h *types.Header, ebftExtra *types.EBFTExtra) error {
	payload, err := rlp.EncodeToBytes(ebftExtra)
	if err != nil {
		return err
	}
