...
func (p *SignedRoundChangePayload) DecodeRLP(stream *rlp.Stream) error {
	...
	if _, err := stream.List(); err != nil {
		log.Trace("[Consensus|EBFT]: Error List() Signed Payload", "err", err)
		return err
	}
	...
	if err != nil {
		log.Error("[Consensus|EBFT]: Error Raw()", "err", err)
		return err
	}
	...
	if _, err = payloadStream.List(); err != nil {
		log.Error("[Consensus|EBFT]: Error List() Payload", "err", err)
		return err
	}

	if err = payloadStream.Decode(&p.Sequence); err != nil {
		log.Error("[Consensus|EBFT]: Error Decode(&m.Sequence)", "err", err)
		return err
	}
	if err = payloadStream.Decode(&p.Round); err != nil {
		log.Error("[Consensus|EBFT]: Error Decode(&m.Round)", "err", err)
		return err
	}
	...
	if size, err = payloadStream.List(); err != nil {
		log.Error("[Consensus|EBFT]: Error List() Prepared", "err", err)
		return err
	}
	if size > 0 {
		if err = payloadStream.Decode(&p.PreparedRound); err != nil {
			log.Error("[Consensus|EBFT]: Error Decode(&m.PreparedRound)", "err", err)
			return err
		}
		if err = payloadStream.Decode(&p.PreparedDigest); err != nil {
			log.Error("[Consensus|EBFT]: Error Decode(&p.PreparedDigest)", "err", err)
			return err
		}
		if err = payloadStream.Decode(&p.HasBadProposal); err != nil {
			log.Error("[Consensus|EBFT]: Error Decode(&p.HasBadProposal)", "err", err)
			return err
		}
	}
	// End Prepared
	...
	log.Trace("[Consensus|EBFT]: Correctly decoded SignedRoundChangePayload", "p", p)
...
func (m *RoundChange) DecodeRLP(stream *rlp.Stream) error {
	...
	if size > 0 {
		...
		if err = payloadStream.Decode(&m.PreparedDigest); err != nil {
			log.Error("[Consensus|EBFT]: Error Decode(&m.PreparedDigest)", "err", err)
			return err
		}
		if err = payloadStream.Decode(&m.HasBadProposal); err != nil {
			log.Error("[Consensus|EBFT]: Error Decode(&m.HasBadProposal)", "err", err)
			return err
		}
	}
	// End Prepared
	...
	if _, size, err = stream.Kind(); err != nil {
		log.Error("[Consensus|EBFT]: Error Kind()", "err", err)
		return err
	}
	if size == 0 {
		...
		if m.PreparedBlock.Hash() != m.PreparedDigest {
			log.Error("[Consensus|EBFT]: Error m.PreparedDigest.Hash() != digest")
	...
	if size == 0 {
		...
		if err = stream.Decode(&m.Justification); err != nil {
			log.Error("[Consensus|EBFT]: Error Decode(&m.Justification)", "err", err)
			return err
		}
	}
