...
func (c *core) broadcastRoundChange(round *big.Int) {
	...
	if cv.Round.Cmp(round) > 0 {
		logger.Error("[Consensus|IBFT]: Invalid past target round", "target", round)
		return
	}
	...
	if err != nil {
		withMsg(logger, roundChange).Error("[Consensus|IBFT]: Failed to encode ROUND-CHANGE message", "err", err)
		return
	}
	signature, err := c.backend.Sign(encodedPayload)
	if err != nil {
		withMsg(logger, roundChange).Error("[Consensus|IBFT]: Failed to sign ROUND-CHANGE message", "err", err)
		return
	}
	...
	if c.IBFTPreparedPrepares != nil {
		...
		withMsg(logger, roundChange).Debug("[Consensus|IBFT]: Extended ROUND-CHANGE message with PREPARE justification", "justification", roundChange.Justification)
	}
	...
	withMsg(logger, roundChange).Trace("[Consensus|IBFT]: Broadcast ROUND-CHANGE message", "payload", hexutil.Encode(data))
	c.cleanLogger.Info("[Consensus|IBFT]: -> Broadcasting ROUND-CHANGE message to validators")

	// Broadcast RLP-encoded message
	if err = c.backend.Broadcast(c.valSet, roundChange.Code(), data); err != nil {
		withMsg(logger, roundChange).Error("[Consensus|IBFT]: Failed to broadcast ROUND-CHANGE message", "err", err)
		return
	}
}
...
func (c *core) handleRoundChange(roundChange *ibfttypes.RoundChange) error {
	...
	if view.Round.Cmp(currentRound) >= 0 {
		...
		if err != nil {
			logger.Warn("[Consensus|IBFT]: Failed to add ROUND-CHANGE message", "err", err)
			return err
		}
	}
	...
	if num == c.valSet.F()+1 {
		...
		logger.Trace("[Consensus|IBFT]: Received F+1 ROUND-CHANGE messages", "F", c.valSet.F())
		c.cleanLogger.Info("[Consensus|IBFT]: <- Received F+1 ROUND-CHANGE messages", "F", c.valSet.F())
		...
		logger.Trace("[Consensus|IBFT]: Received quorum of ROUND-CHANGE messages")
		c.cleanLogger.Info("[Consensus|IBFT]: <- Received quorum of ROUND-CHANGE messages", "count", currentRoundMessages, "quorum", c.QuorumSize())
