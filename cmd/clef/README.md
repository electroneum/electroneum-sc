...
Clef can be used to sign transactions and data and is meant as a(n eventual) replacement for ETN-SC's account management. This allows DApps to not depend on ETN-SC's account management. When a DApp wants to sign data (or a transaction), it can send the content to Clef, which will then provide the user with context and asks for permission to sign the content. If the users grants the signing request, Clef will send the signature back to the DApp.

This setup allows a DApp to connect to a remote Electroneum node and send transactions that are locally signed. This can help in situations when a DApp is connected to an untrusted remote Electroneum node, because a local one is not available, not synchronised with the chain, or is a node that has no built-in (or limited) account management.
...
GLOBAL OPTIONS:
   ...
   --keystore value        Directory for the keystore (default: "$HOME/.electroneum-sc/keystore")
   --configdir value       Directory for Clef configuration (default: "$HOME/.clef")
   --/. value              Chain id to use for signing (52014=mainnet, 5201419=Stagenet, 5201420=Testnet) (default: 52014)
...
The general flow for signing a transaction using e.g. ETN-SC is as follows:
![image](sign_flow.png)

In this case, `etn-sc` would be started with `--signer http://localhost:8550` and would relay requests to `eth.sendTransaction`.
...
* ETN-SC todos
    ...
    - Relay: Etn-sc should be started in `etn-sc --signer localhost:8550`.
    - Currently, the Etn-sc APIs use `common.Address` in the arguments to transaction submission (e.g `to` field). This type is 20 `bytes`, and is incapable of carrying checksum information. The signer uses `common.MixedcaseAddress`, which retains the original input.
    - The Etn-sc API should switch to use the same type, and relay `to`-account verbatim to the external API.
* [x] Storage
    * [x] An encrypted key-value storage should be implemented.
    * See [rules.md](rules.md) for more info about this.
* Another potential thing to introduce is pairing.
  * To prevent spurious requests which users just accept, implement a way to "pair" the caller with the signer (external API).
  * Thus ETN-sc/cpp would cryptographically handshake and afterwards the caller would be allowed to make signing requests.
...
Clef listens to HTTP requests on `http.addr`:`http.port` (or to IPC on `ipcpath`), with the same JSON-RPC standard as Etn-sc. The messages are expected to be [JSON-RPC 2.0 standard](https://www.jsonrpc.org/specification).
  ...
  "params": [
    {
      "accounts": [
        {
          "url": "keystore:///home/bazonk/.electroneum/keystore/UTC--2017-11-20T14-44-54.089682944Z--123409812340981234098123409812deadbeef42",
          "address": "0x123409812340981234098123409812deadbeef42"
        },
        {
          "url": "keystore:///home/bazonk/.electroneum/keystore/UTC--2017-11-23T21-59-03.199240693Z--cafebabedeadbeef34098123409812deadbeef42",
  ...
  "params": [
    {
      ...
      "messages": [
        {
          "name": "message",
          "value": "\u0019Electroneum Signed Message:\n4\u0001\u0002\u0003\u0004",
          "type": "text/plain"
        }
      ],
...
* A Graphical UI MUST show the blocky-identicon for Electroneum addresses.
